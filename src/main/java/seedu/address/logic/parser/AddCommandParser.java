...
public class AddCommandParser implements Parser<AddCommand> {
    ...
    public AddCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_NAME, PREFIX_DOB, PREFIX_PHONE, PREFIX_EMAIL,
                        PREFIX_ADDRESS, PREFIX_BLOODTYPE, PREFIX_APPOINTMENT, PREFIX_NOK, PREFIX_TAG,
                        PREFIX_MEDICAL_HISTORY);

        if (!arePrefixesPresent(argMultimap, PREFIX_NAME, PREFIX_DOB, PREFIX_ADDRESS, PREFIX_PHONE, PREFIX_BLOODTYPE,
                PREFIX_APPOINTMENT) || !argMultimap.getPreamble().isEmpty()) {
        ...
        Optional<String> placeholder = argMultimap.getValue(PREFIX_EMAIL);
        Email email;
        if (placeholder.isPresent()) {
            email = ParserUtil.parseEmail(placeholder.get());
        } else {
        ...
        Person person = new Person(name, dob, phone, email, address, bloodType, appointment, tagList,
